...
// 사용후기 작성 설정에 따른 체크
cm_check_itemuse_write($it_id, $member['mb_id']);
...
if($is_mobile_shop)
    ...
    $url = "./item.php?it_id=$it_id&_=".get_token()."#cit_use";
...
if ($w == "")
{
    ...
    */
    ...
    $sql = "insert {$g5['g5_contents_item_use_table']}
               set it_id = '$it_id',
                   mb_id = '{$member['mb_id']}',
                   is_score = '$is_score',
                   is_name = '$is_name',
...
{
    $sql = " select is_password from {$g5['g5_contents_item_use_table']} where is_id = '$is_id' ";
    $row = sql_fetch($sql);
...
else if ($w == "d")
{
    if (!$is_admin)
    {
        $sql = " select count(*) as cnt from {$g5['g5_contents_item_use_table']} where mb_id = '{$member['mb_id']}' and is_id = '$is_id' ";
        $row = sql_fetch($sql);
        if (!$row['cnt'])
    ...
    // 에디터로 첨부된 이미지 삭제
    $sql = " select is_content from {$g5['g5_contents_item_use_table']} where is_id = '$is_id' and md5(concat(is_id,is_time,is_ip)) = '{$hash}' ";
    $row = sql_fetch($sql);
    ...
    $sql = " delete from {$g5['g5_contents_item_use_table']} where is_id = '$is_id' and md5(concat(is_id,is_time,is_ip)) = '{$hash}' ";
    sql_query($sql);

    $alert_msg = "사용후기를 삭제 하였습니다.";
}

cm_update_use_avg($it_id);
