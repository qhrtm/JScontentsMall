...
if (G5_IS_MOBILE) {
    include_once(G5_MCONTENTS_PATH.'/orderinquiryview.php');
    return;
}
...
$sql = "select * from {$g5['g5_contents_order_table']} where od_id = '$od_id' and mb_id = '{$member['mb_id']}' ";
$od = sql_fetch($sql);
if (!$od['od_id']) {
    alert("조회하실 주문서가 없습니다.", G5_CONTENTS_URL);
}
...
<script src="<?php echo G5_JS_URL; ?>/movie.js"></script>
...
<div id="sod_fin">
    ...
    <section id="sod_fin_list">
        ...
        $sql = " select a.it_id, a.it_name, b.it_contents_type
        ...
        <div class="tbl_head02 tbl_wrap">
            ...
            <tr>
                ...
                <th scope="col" id="th_itst">컨텐츠확인</th>
            </tr>
            </thead>
            <tbody>
            <?php
            for($i=0; $row=sql_fetch_array($result); $i++) {
                ...
                $sql = " select ct_id, it_name, ct_option, ct_qty, ct_price, ct_point, ct_status, io_price, ct_time, ct_ip
                ...
                for($k=0; $opt=sql_fetch_array($res); $k++) {
                    ...
                    if($opt['ct_status'] == '입금') {
                        $uid = md5($opt['ct_id'].$opt['ct_time'].$opt['ct_ip']);
                        $qry = 'od_id='.$od['od_id'].'&amp;ct_id='.$opt['ct_id'];

                        // 세션에 uid 기록
                        set_session('ss_contents_'.$opt['ct_id'].'_uid', $uid);

                        if($row['it_contents_type'] == 2 || $row['it_contents_type'] == 3) {
                            $opt_status = '<a href="'.G5_CONTENTS_URL.'/movie.php?'.$qry.'" class="win_view_movie">동영상보기</a>';
                        } else {
                            $opt_status = '<a href="'.G5_CONTENTS_URL.'/download.php?'.$qry.'">다운로드</a>';
                        }
                    } else {
                        $opt_status = $opt['ct_status'];
                    }
            ...
            <tr>
                ...
                <td headers="th_itname" colspan="6" class="t_l"><a href="./item.php?it_id=<?php echo $row['it_id']; ?>"><?php echo $row['it_name']; ?></a></td>
            </tr>
            <?php } ?>
            <tr>
                <td headers="th_itopt" class="t_l"><?php echo $opt['ct_option']; ?></td>
                <td headers="th_itqty" class="td_mngsmall"><?php echo number_format($opt['ct_qty']); ?></td>
                <td headers="th_itprice" class="td_numbig t_r"><?php echo number_format($opt_price); ?></td>
                <td headers="th_itsum" class="td_numbig t_r"><?php echo number_format($sell_price); ?></td>
                <td headers="th_itpt" class="td_num t_r"><?php echo number_format($point); ?></td>
                <td headers="th_itst" class="td_mngsmall"><?php echo $opt_status; ?></td>
            </tr>
            <?php
                    $tot_point       += $point;
        ...
        <dl id="sod_bsk_tot">
            ...
            <dt class="sod_bsk_ccl">취소금액</dt>
            <dd class="sod_bsk_ccl"><strong><?php echo number_format($od['od_cancel_price']); ?> 원</strong></dd>
            <?php } ?>
    ...
    <div id="sod_fin_view">
        ...
        if ($misu_price == 0 && ($od['od_cart_price'] > $od['od_cancel_price'])) {
            $wanbul = cm_display_price($receipt_price)." (완불)";
            $misu = false; // 미수금 없음
        }
        else
        {
            $wanbul = cm_display_price($receipt_price);
        }
        ...
        if($od['od_receipt_price'] > 0)
            $od_receipt_price = cm_display_price($od['od_receipt_price']);
        else {
            if($misu_price == 0) {
                $od_receipt_price = cm_display_price($od['od_receipt_cash'] + $od['od_receipt_point']);
            } else {
        ...
        if($od['od_settle_case'] == '신용카드') {
            ...
            $app_no = $od['od_tno'];
            $disp_bank = true;
        } else if($od['od_settle_case'] == '무통장') {
            $disp_bank = true;
        }
        ?>

        <section id="sod_fin_pay" class="sod_fin_if">
            <h3>결제정보</h3>
            ...
            <div class="tbl_head01 tbl_wrap">
                ...
                <?php
                if(!cm_is_null_time($od['od_receipt_time']))
                {
                ?>
                <tr>
                    ...
                    <td>
                        ...
                        if($od['od_settle_case'] == '휴대폰')
                        {
                            if($od['od_pg'] == 'lg') {
                                ...
                                $hp_receipt_script = 'window.open(\''.G5_CM_BILL_RECEIPT_URL.'mcash_bill&tno='.$od['od_tno'].'&order_no='.$od['od_id'].'&trade_mony='.$od['od_receipt_price'].'\', \'winreceipt\', \'width=500,height=690,scrollbars=yes,resizable=yes\');';
                            }
                        ?>
                        ...
                        if($od['od_settle_case'] == '신용카드')
                        {
                            if($od['od_pg'] == 'lg') {
                                ...
                                $card_receipt_script = 'window.open(\''.G5_CM_BILL_RECEIPT_URL.'card_bill&tno='.$od['od_tno'].'&order_no='.$od['od_id'].'&trade_mony='.$od['od_receipt_price'].'\', \'winreceipt\', \'width=470,height=815,scrollbars=yes,resizable=yes\');';
                            }
                        ?>
                ...
                <tr>
                    ...
                    if ($od['od_cash'])
                    {
                        if($od['od_pg'] == 'lg') {
                            ...
                            $cash = unserialize($od['od_cash_info']);
                            $cash_receipt_script = 'window.open(\''.G5_CM_CASH_RECEIPT_URL.$setting['de_kcp_mid'].'&orderid='.$od_id.'&bill_yn=Y&authno='.$cash['receipt_no'].'\', \'taxsave_receipt\', \'width=360,height=647,scrollbars=0,menus=0\');';
                        }
                    ?>
                    ...
                    {
                    ?>
                        <a href="javascript:;" onclick="window.open('<?php echo G5_CONTENTS_URL; ?>/taxsave.php?od_id=<?php echo $od_id; ?>', 'taxsave', 'width=550,height=400,scrollbars=1,menus=0');" class="btn_frmline">현금영수증을 발급하시려면 클릭하십시오.</a>
                    <?php } ?>
                    </td>
                </tr>
                <?php
        ...
        <section id="sod_fin_orderer" class="sod_fin_if">
            <h3>구매하신 분</h3>
    ...
    <section id="sod_fin_tot">
        ...
        <ul>
            <li>
                총 구매액
                <strong><?php echo cm_display_price($od['od_cart_price']); ?></strong>
            </li>
            <?php
            if ($misu_price > 0) {
            echo '<li>';
    ...
    <?php if ($od['od_settle_case'] == '가상계좌' && $od['od_misu'] > 0 && $setting['de_card_test'] && $is_admin && $od['od_pg'] == 'kcp') {
    ...
    <div class="tbl_frm01 tbl_wrap od_acc">
        ...
        <tr>
            ...
            <td><input type="text" name="e_trade_no" value="<?php echo $od['od_tno']; ?>" class="frm_input"></td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td><input type="text" name="req_name" value="<?php echo $od['od_deposit_name']; ?>" class="frm_input"></td>
        </tr>
        <tr>
            <th scope="col"><label for="noti_url">입금통보 URL</label></th>
            <td><input type="text" name="noti_url" value="<?php echo G5_CONTENTS_URL; ?>/settle_kcp_common.php" class="frm_input" size="70"></td>
        </tr>
        </tbody>
        </table>
