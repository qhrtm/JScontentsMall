...
<!-- 컨텐츠몰 카테고리 시작 { -->
<nav id="gnb">
    <h3 class="accessibility">컨텐츠몰 카테고리</h3>
    <ul class="gnb">
        <?php
        // 1단계 분류 판매 가능한 것만
        ...
        ?>
        <li>
            <a href="<?php echo G5_CONTENTS_URL.'/list.php?ca_id='.$row['ca_id']; ?>" class="mpr "><?php echo $row['ca_name']; ?></a>
            <?php
            for ($j=0; $row2=sql_fetch_array($result2); $j++)
            {
            if ($j==0) echo '<ul class="gnbSub">';
            ?>
                <li><a href="<?php echo G5_CONTENTS_URL; ?>/list.php?ca_id=<?php echo $row2['ca_id']; ?>"><?php echo $row2['ca_name']; ?></a></li>
            <?php }
            if ($j>0) echo '</ul>';
            ?>
        </li>
